...
def draw_plot_func(values, name_classes, plot_title, x_label, output_path, tick_font_size = 12, plt_show = True):
    ...
    for i, val in enumerate(values):
        ...
        t = axes.text(val, i, str_val, color='royalblue', va='center', fontweight='bold')
        if i == (len(values) - 1):
            adjust_axes(t, fig, axes)
...
def adjust_axes(text, fig, axes):
    # 调整坐标轴
    r = fig.canvas.get_renderer()
    ex = text.get_window_extent(renderer=r)
    text_width = ex.width / fig.dpi
    axes_width = axes.get_window_extent(renderer=r).width / fig.dpi
    ratio = axes_width / text_width

    if ratio < 1.05:
        diff_width = (text_width - axes_width) / 2
        axes.set_xlim(axes.get_xlim()[0] - diff_width, axes.get_xlim()[1] + diff_width)
