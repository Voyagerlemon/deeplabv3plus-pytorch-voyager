...
from attention.cbam import CBAMLayer
from attention.ecanet import ECALayer

from nets.xception import xception
from nets.mobilenetv2 import mobilenetv2
from nets.mobilenetv3 import build_mobilenetv3_large

attention_block = [CALayer, CBAMLayer, ECALayer, SELayer]
...
#-----------------------------------------------------------------------------------#
# ASPP(空洞空间卷积池化金字塔)特征提取模块 --> 深度卷积神经网络(主干网络)处理后的模块
# 利用不同膨胀率的膨胀卷积进行特征提取
#-----------------------------------------------------------------------------------#
class ASPP(nn.Module):
	...
	def __init__(self, dim_in, dim_out, rate=1, bn_mom=0.1):
		...
		self.branch1 = nn.Sequential(
                ...
                #------------------------------------------------------------#
                # inplace=True --> 原地操作, 即将结果覆盖到输入张量中, 节省内存
                #------------------------------------------------------------#
...
class DeepLab(nn.Module):
    def __init__(self, num_classes, backbone="mobilenet", pretrained=True, downsample_factor=16, att=0):
        #----------------------------------------#
        # 使用super()初始化父类的__init__构造方法
        #----------------------------------------#
