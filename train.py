...
if __name__ == "__main__":
    ...
    num_classes     = 19
    #---------------------------------#
    # 主干网络：mobilenet  xception
    #---------------------------------#
    backbone        = "mobilenet"
    #----------------------------------------------------------------------------------------------------------------------------#
    #   pretrained --> 是否使用主干网络的预训练权重，此处使用的是主干的权重，因此是在模型构建的时候进行加载的。
    #   1、如果设置了model_path，则主干的权值无需加载，pretrained的值无意义。
    #   2、如果不设置model_path，pretrained = True，此时仅加载主干开始训练。
    ...
    #---------------------------------------------------------#
    #  downsample_factor -->下采样的倍数8(3次下采样)、16(4次下采样)
    ...
    #-----------------------------------------------------------------------------#
    #   att       选择注意力机制, 共有四种可选
    #   att = 1   CA位置注意力机制   att = 2 CBAM通道注意力机制和空间注意力机制的结合
    #   att = 3   ECANet注意力机制   att = 4 SENet注意力机制
    #-----------------------------------------------------------------------------#
    att                 = 0
    #------------------------------------------------------#
    #   设置用到的显卡
    ...
    model   = DeepLab(num_classes=num_classes, backbone=backbone, downsample_factor=downsample_factor, pretrained=pretrained, att=att)
    if not pretrained:
        weights_init(model)
    if model_path != '':
        if local_rank == 0:
